...
export const AISymptomCheckerForBooking = ({ onComplete, onBack }: AISymptomCheckerProps) => {
  ...
  const fetchSymptomCategories = async () => {
    try {
      ...
      // Fallback categories
      setCategories([
        { category: "General", symptoms: ["Fever", "Fatigue", "Weakness", "Weight loss", "Chills"] },
        { category: "Head & Neurological", symptoms: ["Headache", "Dizziness", "Confusion", "Numbness"] },
        { category: "Respiratory", symptoms: ["Cough", "Shortness of breath", "Wheezing", "Chest tightness"] },
        { category: "Cardiovascular", symptoms: ["Chest pain", "Palpitations", "Swelling in legs"] },
        { category: "Gastrointestinal", symptoms: ["Nausea", "Vomiting", "Diarrhea", "Abdominal pain"] },
      ]);
    }
  };
  ...
  const handleAssess = async () => {
    ...
    try {
      ...
      toast.error("Failed to complete AI assessment. Proceeding with basic booking.");
      // Fallback - proceed without AI assessment
      handleProceedWithoutAI();
    } finally {
      setLoading(false);
    }
  };

  const handleProceedWithoutAI = () => {
    onComplete({
      symptoms: selectedSymptoms,
      severity,
      description,
  ...
  const handleProceedWithAI = () => {
    ...
    // Map care pathway to specialization
    ...
    onComplete({
      ...
      severity,
      description,
      recommendedSpecialization: result.recommended_specialization 
        ? specMap[result.recommended_specialization] || result.recommended_specialization 
        : null,
      aiAssessment: result,
    });
  };
    ...
    <div className="space-y-6">
      {step < 4 && (
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-2">Step {step} of 3 - AI Symptom Assessment</p>
        </div>
      )}
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Select all that apply. Our AI will help assess your needs.</CardDescription>
          </CardHeader>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.symptoms.map((symptom) => (
                      <Badge
                        ...
                        className="cursor-pointer py-1.5 px-3 text-xs sm:text-sm"
      ...
      {step === 2 && (
        <Card>
          ...
          <CardContent className="space-y-6">
            <RadioGroup value={severity} onValueChange={(v) => setSeverity(v as any)} className="space-y-3">
      ...
      {step === 3 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Any additional details help our AI provide a better assessment
            </CardDescription>
          </CardHeader>
      ...
      <div className="flex justify-between">
        ...
        {step === 4 && result && result.urgency !== "emergency" && (
          <Button onClick={handleProceedWithAI}>
...
export default AISymptomCheckerForBooking;
